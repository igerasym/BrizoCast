...
class GeminiProvider:
    ...
    def _build_prompt(region: str, activity_key: str) -> str:
        ...
        return (
            f"You are a professional surf forecaster setting ABOVE-AVERAGE alert "
            f"thresholds for {region!r} (activity: {activity_key!r}).\n\n"
            "## Goal\n"
            "Set thresholds for ABOVE-AVERAGE sessions — conditions that are "
            "noticeably better than a typical day in the region. "
            "An alert should feel like 'this is genuinely good, go surf NOW.'\n"
            "NOT just 'there are waves'. Most days in the region should NOT trigger an alert.\n\n"
            "## Parameters\n"
            "- min_wave_m: Minimum wave height for an ABOVE-AVERAGE session. "
            "Set this HIGHER than the regional average. "
            "If average is 0.8m, set 1.2m. If average is 1.5m, set 2.0m.\n"
            "- max_wave_m: Upper limit before conditions get dangerous/closing out.\n"
            "- min_period_s: Minimum swell period for quality surf. "
            "period <7s = wind chop (skip), 8-10s = decent, 10s+ = quality swell. "
            "Set above the regional average.\n"
            "- max_wind_kmh: Maximum wind for acceptable conditions. "
            "Prefer 15-20 km/h (light/moderate). Strong winds = messy surface.\n"
            "- preferred_wind_dir_deg: Direction wind blows FROM that's offshore.\n"
            "- preferred_swell_dir_deg: Optimal swell direction for the region.\n\n"
            "## Scoring (0-100)\n"
            "  wave_height 30%, swell_period 25%, wind_speed 20%, "
            "wind_dir 15%, swell_dir 10%\n"
            "Categories: IGNORE 0-49 | RIDEABLE 50-69 | GOOD 70-84 | EXCELLENT 85+\n\n"
            "## min_alert_score — ABOVE AVERAGE target\n"
            "Alert fires ONLY when score exceeds this. "
            "Target: only the TOP 20-25%% of days in the region trigger an alert.\n"
            "- Cold/rare surf (Baltic, N.Europe): 65-70 — even mediocre sessions are noteworthy\n"
            "- Atlantic Europe (Portugal, France, UK): 72-78 — good swell days only\n"
            "- Warm/consistent (Canary Islands, Med): 76-82 — above-average days only\n"
            "- World-class (Bali, Hawaii, Maldives): 82-88 — only exceptional days\n"
            "Set min_alert_score high enough that alerts feel SPECIAL, not routine.\n\n"
            "## Rules\n"
            "- Thresholds must be ABOVE the regional daily average, not AT it.\n"
            "- A 0.8m wave with 5s period is NOT worth alerting. Ignore it.\n"
            "- For consistent surf regions: raise score threshold, not just wave height.\n\n"
